...
fn grouped_metric_and_labels(sample: &Sample, grouping: Option<&Grouping>) -> (String, Vec<Label>) {
    let metric = match grouping {
        Some(grouping)
            if !grouping.without && grouping.labels.iter().any(|label| label == "__name__") =>
        {
            sample.metric.clone()
        }
        _ => String::new(),
