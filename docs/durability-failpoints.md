...
| WAL truncation/reset | `WalDurabilityFailpoint::ResetAfterMarkerSync`, after `TSH2` with `H = R = max(last appended, (active segment, 0))` is durable but before any WAL frame is removed; `WalDurabilityFailpoint::ResetAfterTruncate`, after the empty active replacement is synchronized and before older segment removal and the final WAL-directory sync | `engine::wal::tests::reset_marker_sync_failpoint_recovery_finishes_the_authorized_reset`; `engine::storage_engine::tests::durability_failpoints::wal_reset_after_truncate_failpoint_reopens_from_published_segment` | Reopen treats verified `R` as reset authorization, finishes or accepts the interrupted reset, and restores append/durable floors. The already `Durable` sample remains available from its clean published segment; a later commit preserves `R`, advances `H`, and still reopens with exact values. |
...
| Snapshot final rename | `ChunkStorage::set_snapshot_pre_publication_hook`, after staging is fully synchronized and before final source re-attestation plus handle-relative no-replace publication; the existing copy observer installs a raced destination, and the directory-sync hook fails after a successful no-replace rename | `engine::storage_engine::tests::durability_failpoints::snapshot_pre_final_rename_failpoint_uses_platform_safe_cleanup_and_reopens_source`; `engine::storage_engine::tests::persistence_background::snapshot_destination_created_during_copy_is_not_replaced`; `engine::storage_engine::tests::persistence_background::snapshot_publication_sync_failure_retains_the_visible_destination` | A pre-rename error publishes nothing while the `Durable` source reopens exactly. Windows removes the fully identity-captured staging tree through the verified handles; portable Unix reports its lack of identity-conditioned unlink and retains the tree. The atomic no-replace rename preserves the other owner's destination, with the same platform-safe losing-stage outcome. A post-rename sync failure retains the visible destination with an explicit indeterminate-durability error, preserving any consumer-created descendant. |
