...
| Binary/library size and dependency footprint | **Partial** | Current package dry runs contain 324 core, 14 protocol, 84 server, 12 testkit, and 18 UniFFI files. Exact extracted archives compile offline against the packaged foundations, as recorded in the [`roadmap checklist`](goal-progress.md#roadmap-checklist). Binary byte sizes and dependency-footprint measurements are absent. |
...
| Support-bundle composition admits one root execution before child collection, keeps an exact same-execution guard around every completed child response, caps aggregate retained child bodies/headers at 16 MiB, reserves root bytes plus 256 KiB scratch without double-counting child bytes, avoids a duplicate parsed JSON tree, and caps and charges the final encoded HTTP response body once at 16 MiB while retaining modeled body/header bytes through response construction. Child sources still charge canonical logical returned work. Tenant override decoding is capped at 16 KiB and no longer clones the HTTP request body. This is an adapter-layer modeled bound, not a child-producer or process-RSS bound. | [`Phase 2`](goal-progress.md#phase-2--finite-resource-profiles-and-admission-control-in-progress) records 14 focused support-bundle tests covering exact/one-under returned-byte and memory admission, cumulative child guards and ceilings, bounded tenant resolution and text fallback, one-query cluster compatibility, cancellation/error mapping, and zero residual resources; the earlier default and no-default 9-test rows remain recorded separately. | **Deterministic boundary evidence only**; there is no support-bundle capacity or RSS row. Bounded tenant/actor and synthetic-request setup before the parent reservation, legacy child snapshot/serialization transients, and transport/runtime allocations remain excluded; direct TSDB and rebalance production reuse the root execution. |
| One admitted `/metrics` execution bounds complete metric enumeration, scrape-owned dynamic projections, hotspot transformation, exact returned bytes, and a two-pass 1 MiB normal response body; failures return a fixed Prometheus-parseable fallback bounded to 4 KiB. | [`Phase 2`](goal-progress.md#phase-2--finite-resource-profiles-and-admission-control-in-progress) records exact/one-under projection, returned-byte, response-memory, cancellation, fallback, and zero-residual tests across storage and cluster collectors. | **Deterministic boundary evidence only**; there is no scrape-latency, requested-heap, or RSS capacity row. Retained hotspot state, other status producers, and transport/runtime allocations remain outside this claim. |
...
The CI `quick` BPP job is an execution and health smoke. It still emits the separate
`TARGET_CHECK`, but the current 0.75/1.0 persisted-size target is report-only there; regression
thresholds remain open as described above.

The quick Criterion smoke runs all insert/select cases, including the explicit finite
million-point select envelope, plus the 64-segment persisted-refresh case. The 256- and
1,024-segment refresh cases remain in `scripts/measure_perf.sh full`; the cached quick comparison
filters its report to the cases that actually ran.
The direct/default comprehensive Criterion group retains older eager high-cardinality fixtures and
is not qualified as a passing finite-profile suite; use the versioned script modes for the
maintained three-group performance matrix.
