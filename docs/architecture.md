...
| `tsink-compaction` | At least one local lane compactor | 1 | 5 s; delete publication may unpark it sooner | `park` provides one coalescing wake token. Numeric and blob lanes alternate across wakes. Before planning, a path-scaled, shared-memory-accounted post-flush clean-fence cursor with 64 KiB directory-stream scratch consumes one raw namespace entry per wake and requires a generation-stable terminal probe; marker publication invalidates it. The lane replacement-recovery cursor likewise consumes one namespace entry or one admitted marker and stops; marker decode has fixed 4 MiB/16K-record caps plus conservative decoded-memory admission. Ordinary planning selects at most eight source segments. Governed filesystem reconciliation remains a separate complete-work residual. |
