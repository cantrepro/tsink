...
pub(super) struct ClusterMetrics<'a> {
    pub write_labeled: &'a WriteRoutingLabeledMetricsExpositionSnapshot,
    pub fanout: ReadFanoutMetricsSnapshot,
    pub fanout_labeled: &'a ReadFanoutLabeledMetricsExpositionSnapshot,
    pub read_planner: ReadPlannerMetricsSnapshot,
    pub read_planner_labeled: &'a ReadPlannerLabeledMetricsExpositionSnapshot,
    pub outbox: OutboxMetricsSnapshot,
    pub outbox_peers: &'a OutboxMetricsExpositionSnapshot,
    pub control: &'a ControlLivenessSnapshot,
    pub handoff: &'a ClusterHandoffSnapshot,
    pub digest: &'a DigestExchangeMetricsSnapshot,
    pub rebalance: &'a RebalanceSchedulerMetricsSnapshot,
    pub hotspot: &'a ClusterHotspotSnapshot,
}

pub(super) fn append_metrics(body: &mut dyn MetricsBodyWriter, metrics: ClusterMetrics<'_>) {
    ...
    append_cluster_outbox_metrics(body, metrics.outbox, metrics.outbox_peers);
    ...
    append_cluster_rebalance_metrics(body, metrics.rebalance, metrics.handoff);
...
fn append_cluster_fanout_metrics(
    ...
    labeled: &ReadFanoutLabeledMetricsExpositionSnapshot,
) {
    body.push_str(
...
fn append_cluster_read_planner_metrics(
    ...
    labeled: &ReadPlannerLabeledMetricsExpositionSnapshot,
) {
    body.push_str(
    ...
    for operation in labeled.operations.iter().flatten() {
...
fn append_cluster_outbox_metrics(
    ...
    peers: &OutboxMetricsExpositionSnapshot,
) {
    body.push_str(
    ...
    for peer in peers.peers.iter().filter(|peer| peer.stalled.is_some()) {
        let node_id = prometheus_escape_label_value(&peer.node_id);
        metrics_write!(
            body,
...
fn append_cluster_control_metrics(
    body: &mut dyn MetricsBodyWriter,
    snapshot: &ControlLivenessSnapshot,
) {
    body.push_str(
...
fn append_cluster_handoff_metrics(
    body: &mut dyn MetricsBodyWriter,
    snapshot: &ClusterHandoffSnapshot,
) {
    body.push_str(
...
fn append_cluster_digest_metrics(
    body: &mut dyn MetricsBodyWriter,
...
fn append_cluster_rebalance_metrics(
    body: &mut dyn MetricsBodyWriter,
    ...
    for job in handoff.shards.iter().filter(|job| job.phase.is_active()) {
...
fn append_cluster_hotspot_metrics(
    body: &mut dyn MetricsBodyWriter,
    snapshot: &ClusterHotspotSnapshot,
) {
    body.push_str(
